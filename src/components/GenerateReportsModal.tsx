...
export default function GenerateReportsModal({ 
  ...
  const generateMonthlySalesReport = () => {
    ...
    doc.setFontSize(10);
    doc.text(`Total Sales: Rs. ${totalSales.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, 20, 80);
    doc.text(`Number of Orders: ${currentMonthSales.length}`, 20, 90);
    doc.text(`Average Order Value: Rs. ${avgOrderValue.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, 20, 100);
  ...
  const generateInventoryReport = () => {
    ...
    doc.text(`Total Inventory Value: Rs. ${totalValue.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, 20, 90);
    ...
    const stockData = inventory.map(item => [
      ...
      item.name,
      item.quantity,
      `Rs. ${item.costPrice.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
      `Rs. ${(item.quantity * item.costPrice).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
    ]);
  ...
  const generateInvoiceReport = () => {
    ...
    doc.setFontSize(10);
    doc.text(`Total Outstanding: Rs. ${totalOutstanding.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, 20, 80);
